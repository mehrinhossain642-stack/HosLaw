...
export default function Home() {
  ...
  const ask = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: 'bot',
          text: error.message || 'Could not connect to the server. Make sure the backend is running.',
        },
      ]);
    }
  ...
  if (!nameSet) {
    return (
      <main
        ...
        style={{
          backgroundImage:
            'radial-gradient(ellipse at 20% 50%, rgba(29,78,216,0.15) 0%, transparent 60%), radial-gradient(ellipse at 80% 20%, rgba(30,58,95,0.2) 0%, transparent 50%)',
        }}
      >
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 space-y-4">
            ...
            <input
              ...
              onKeyDown={(e) =>
                e.key === 'Enter' &&
                nameInput.trim() &&
                (setUserName(nameInput.trim()), setNameSet(true))
              }
  ...
  return (
    <main
      className="min-h-screen bg-slate-950 text-white flex flex-col items-center"
      style={{
        backgroundImage:
          'radial-gradient(ellipse at 20% 50%, rgba(29,78,216,0.12) 0%, transparent 60%), radial-gradient(ellipse at 80% 20%, rgba(30,58,95,0.18) 0%, transparent 50%), radial-gradient(ellipse at 50% 100%, rgba(15,23,42,0.8) 0%, transparent 70%)',
      }}
    >
      <div
